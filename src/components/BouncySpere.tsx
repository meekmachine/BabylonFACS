...
const BouncySphere: FC<BouncySphereProps> = ({ fontsReady, position, name, color }) => {
  ...
  useEffect(() => {
    const applyForceFromMouseMove = (event: MouseEvent) => {
      const scene = sphereRef.current?.getScene();
      if (!scene) return;

      // Convert screen coordinates to Babylon's normalized device coordinates
      const pickResult = scene.pick(
        scene.pointerX,
        scene.pointerY
      );

      if (pickResult?.hit && pickResult.pickedMesh === sphereRef.current) {
        applyForceToSphere(pickResult.pickedPoint!, pickResult.ray!.direction);
      }
    };

    window.addEventListener('mousemove', applyForceFromMouseMove);

    return () => {
      window.removeEventListener('mousemove', applyForceFromMouseMove);
    };
  }, []);

  const applyForceToSphere = (pickedPoint: Vector3, direction: Vector3) => {
    if (sphereRef.current?.physicsImpostor) {
      const forceMagnitude = 5; // Adjust this value for stronger/weaker force
      const force = direction.scale(forceMagnitude);
      sphereRef.current.physicsImpostor.applyImpulse(force, pickedPoint);
    }
  };
